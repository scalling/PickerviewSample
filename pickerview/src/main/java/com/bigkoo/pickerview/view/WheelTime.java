...
import com.bigkoo.pickerview.listener.OnItemChangeListener;
...
public class WheelTime {
    ...
    private Calendar checkStartDate;//不容许超出范围的开始时间  null不检测
    private Calendar checkEndDate;//不容许超出范围的结束时间时间 null默认为当前系统时间
    private boolean isCheck=false;//是否检查超出范围的数据 默认不检测
    ...
    private void check(){
        if(isCheck) {
            ...
            if (WheelTime.this.checkEndDate != null)
                endDate = WheelTime.this.checkEndDate;
            ...
            selDate.set(year, month, day, hours, min, s);
            long selDataTime = selDate.getTimeInMillis();
            if (endDate!=null&&selDataTime > endDate.getTimeInMillis()) {
                  setTime(endDate);
            }
            if (checkStartDate!=null&&selDataTime < checkStartDate.getTimeInMillis()) {
                setTime(checkStartDate);
            }
        }

    }
    private void  setTime(Calendar data){
        wv_year.setCurrentItem(data.get(Calendar.YEAR)-startYear);
        wv_month.setCurrentItem(data.get(Calendar.MONTH));
        wv_day.setCurrentItem(data.get(Calendar.DAY_OF_MONTH));
        wv_hours.setCurrentItem(data.get(Calendar.HOUR_OF_DAY));
        wv_mins.setCurrentItem(data.get(Calendar.MINUTE));
        wv_seconds.setCurrentItem(data.get(Calendar.SECOND));
    }
    ...
    public void setCheckStartDate(Calendar checkStartDate) {
        this.checkStartDate = checkStartDate;
    }

    public void setCheckEndDate(Calendar checkEndDate) {
